...
<xsl:call-template name="actor-transaction-diagram"/>
    ...
    <xsl:template name='transaction2'>
        ...
        <xsl:call-template name="figureLink">
            ...
            <xsl:with-param name="file">transaction-<xsl:value-of select="@id"/>-uc.svg</xsl:with-param>
...
&lt;tbody></xsl:text>
        <xsl:for-each select="//ig:actor[@id = $actorids]">
            ...
            <xsl:text>&lt;/a>&lt;/td>
&lt;td></xsl:text><xsl:choose>
	<xsl:when test='(ig:supports-tx|ig:requires-tx)[@transaction=$tx/@id]/ig:role'>
		<xsl:value-of select='(ig:supports-tx|ig:requires-tx)[@transaction=$tx/@id]/ig:role'/>
	</xsl:when>
	<xsl:when test='(ig:supports-tx|ig:requires-tx)[@transaction=$tx/@id]/@role'>
		<xsl:variable name='role' select='(ig:supports-tx|ig:requires-tx)[@transaction=$tx/@id]/@role'/>
		<xsl:value-of select='//ig:actor[@id=$role]/(ig:supports-tx|ig:requires-tx)[@transaction=$tx/@id]/ig:role'/>
	</xsl:when>
</xsl:choose>
&lt;/td>
&lt;/tr>
...
### Interactions
        ...
        <xsl:call-template name="figureLink">
            ...
            <xsl:with-param name="file">transaction-<xsl:value-of select="@id"/>-seq.svg</xsl:with-param>
            <xsl:with-param name="number">2.<xsl:value-of select="position()"/>-2</xsl:with-param>
        </xsl:call-template>


        <xsl:for-each select=".//(ig:message[not(@ref)]|ig:result)">
...
See the following CapabilityStatement resources for conformance requirements:
    ...
    <xsl:template match="ig:operation">
        ...
        <xsl:if test="@client">
            ...
            <xsl:text>The </xsl:text><xsl:value-of select='//ig:actor[@id=$from]/ig:name'/>
       	...
       	<xsl:variable name='to' select='ancestor::ig:message[1]/@to'/>
        <xsl:text>The </xsl:text><xsl:value-of select='//ig:actor[@id=$to]/ig:name'/>
        <xsl:text> </xsl:text><xsl:apply-templates select="@expect"/>
        <xsl:text> support the FHIR </xsl:text><xsl:value-of select="@name"/><xsl:text> operation on </xsl:text>
        <xsl:apply-templates select="@resources"/>
        <xsl:if test='ig:parameter|ig:group'>
            <xsl:text> with the following parameters</xsl:text>
        </xsl:if>
...
&lt;/table>&#xA;</xsl:text>
    ...
    <xsl:template match="ig:message" mode="interaction-diagram">
        ...
        <xsl:variable name='ref' select='@ref'/>
		<xsl:variable name='msg' select='.[not(@ref)]|../ig:message[@id=$ref]'/>
        <xsl:text>"</xsl:text>
        <xsl:value-of select="@from"/>
        <xsl:text>" -> "</xsl:text>
        <xsl:value-of select="@to"/>
        <xsl:text>" : </xsl:text>
        <xsl:value-of select="concat($number,'. ',$msg/ig:name)"/>
        <xsl:text>&#xA;</xsl:text>
        ...
        <xsl:apply-templates select="$msg/ig:message|$msg/ig:result" mode="interaction-diagram"/>
        <xsl:if test="not($msg/ig:result) and not(string(@from) = string(@to))">return&#xA;</xsl:if>
        <xsl:if test="not($msg/self::ig:message)">deactivate "<xsl:value-of select="@from"/>"&#xA;</xsl:if>
        <xsl:if test="not(string(@from) = string(@to))">
...
&lt;td>
&lt;a href='transaction-<xsl:value-of select="$trans/@id"/>.html'>
...
&lt;/table>
    ...
    <xsl:template name='transaction-descriptions'>
        ...
        <xsl:for-each select="/ig:profile/ig:transaction">
            ...
            <xsl:text>&#xA;</xsl:text>
            <xsl:apply-templates select="ig:description|ig:overview"/>
            <xsl:text>&#xA;For more details see the detailed [transaction description](transaction-</xsl:text><xsl:value-of select="@id"/><xsl:text>.html)&#xA;</xsl:text>
        </xsl:for-each>
    </xsl:template>
   ...
   **AND  **  C = A<br/>
        ...
        <xsl:for-each select="$base/..//ig:actor[(@id=$fromActors or @id=$toActors) and $hasOperations][not($isActor) or .=$isActor]">
            ...
            <xsl:variable name="name">
                ...
                <xsl:if test='not($isActor)'>
                    <xsl:text>-</xsl:text>
                    <xsl:value-of select="translate($tx/ig:transaction/@id,' ','')"/>
                </xsl:if>
    ...
    <xsl:template name='rest-resource-requirements'>
        ...
        <xsl:for-each select="$resourceNames">
            ...
            <xsl:variable name='allGroups' select='$operations[@name="search"]/ig:group'/>
            <xsl:variable name='groups' select='$allGroups/ig:group[not(@ref)]|$operations[1]/ancestor::ig:profile//ig:group[@id = $allGroups/@ref]'/>
